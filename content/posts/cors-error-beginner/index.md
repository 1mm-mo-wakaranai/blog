...
fetchでデータ取ろうとしたら赤いエラーが出た…。CORSって何なの？
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
開発中あるあるだね。CORSはブラウザのセキュリティ機能で、別のサーバーへのアクセスをブロックしてるんだ。原因と直し方を説明するね。
{{< /chat >}}

![CORSエラー 理解度の変化](images/comparison-before-after.png)
...
![CORSエラーが起きる仕組み](images/point-cors-mechanism.png)
...
![CORSエラーの解決方法4パターン](images/step-solutions.png)
...
![CORSエラーのデバッグ手順](images/step-debug.png)
...
正直に言うと、筆者もCORSエラーで丸1日潰したことがあります。

### ハマり1: フロントエンドのコードを何度も書き直した

最初にCORSエラーに遭遇したとき、「fetchの書き方が間違っているのでは？」と思い、フロントエンドのコードを何度も書き直しました。ヘッダーを追加したり、modeを変えたり…。結局2時間無駄にしてから「これはサーバー側の問題だ」と気づきました。

**気づき:** CORSエラーはブラウザが出すエラーだけど、解決するのはサーバー側。フロントエンドのコードをいくら変えても直らない。

### ハマり2: `cors()` を入れる位置を間違えた

Express.jsで `app.use(cors())` を追加したのに、まだエラーが出る。30分悩んだ末に気づいたのは、`cors()` をルーティングの**後**に書いていたこと。ミドルウェアはルーティングの**前**に書かないと効きません。
...
### ハマり3: プリフライトリクエストの存在を知らなかった

GETリクエストでは動くのに、POSTリクエストだとCORSエラーが出る。原因は、`Content-Type: application/json` を指定したPOSTリクエストでは「プリフライトリクエスト（OPTIONS）」が飛ぶこと。サーバー側でOPTIONSメソッドに対応していなかったのが原因でした。

**改善:** それ以降、CORS設定をするときは必ず「GET以外のメソッドでもテストする」ようにしています。Postmanで動いてもブラウザで動かないケースがあるので、必ずブラウザからテストすることを習慣にしました。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
先輩でもそんなにハマるんだ…。私だけじゃないって分かって安心した！
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
CORSは最初みんなハマるポイントだよ。「サーバー側の問題」って分かれば、解決は早いんだ。
{{< /chat >}}
...
{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
サーバー側の設定だったんだ…！フロントだけじゃ直せないのね。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
そう、ブラウザが表示するエラーだけど解決はサーバー側。開発中はプロキシを使うのが一番手軽だよ。
{{< /chat >}}
