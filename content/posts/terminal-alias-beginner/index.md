...
### Q: エイリアスを削除するにはどうすればいいですか？

A: 設定ファイル（`.zshrc` / `.bashrc` / `$PROFILE`）から該当のエイリアス行を削除して、`source`を実行（またはターミナルを再起動）してください。一時的に削除するだけなら、`unalias gs`（Mac/Linux）で現在のセッションからエイリアスを解除できます。
...
- Git系のエイリアスが最も効果的（gs, ga, gc, gp）
- 毎日使うコマンドから設定していくのがおすすめ
- 設定ファイルを編集したら `source` を忘れずに
- 既存コマンドとの名前被りに注意
- チームで作業するときはフルコマンドで共有する

---

## 筆者のおすすめエイリアス設定ファイル（テンプレート）

最後に、筆者が実際に使っている.zshrcのエイリアス部分をテンプレートとして紹介します。コピペして使ってください。
...
このテンプレートをベースに、自分の作業に合わせてカスタマイズしていくのがおすすめです。

### エイリアスの育て方

最初から完璧なエイリアス設定を目指す必要はありません。筆者のおすすめのアプローチ：

1. **最初は3つだけ**（`gs`、`ga`、`gp`）設定する
2. **1週間使ってみて**「これも短縮したい」と思ったコマンドを追加
3. **1ヶ月後**に不要なエイリアスを整理する
4. **チーム内で便利だったエイリアスを共有**する（READMEに書いておく程度でOK）

「便利そうだから全部設定しよう」と一気にやると、覚えきれずに結局使わなくなります。自分の作業で実際に繰り返し打っているコマンドから少しずつ設定していくのがコツです。
