...
![Dockerの基本概念](images/point-docker-concepts.png)
...
![Dockerと仮想マシンの違い](images/comparison-docker-vm.png)
...
## 筆者がDockerを使い始めて変わったこと

実際にDockerを導入してから感じた変化を正直に書きます。

### 環境構築の時間が激減した

以前はチームに新メンバーが入るたびに「Python入れて、PostgreSQL入れて、Redis入れて…」と半日かけて環境構築していました。Docker導入後は `docker-compose up` の一発で完了。所要時間5分。

### 「動かない」トラブルが消えた

「自分のMacでは動くけど、同僚のWindowsでは動かない」が月に2〜3回あったのが、Docker導入後はゼロになりました。全員が同じコンテナで開発するので、環境差異が原因のバグが発生しない。

### 最初の1週間は苦労した

正直、最初はDockerfileの書き方が分からず苦労しました。特に「レイヤーキャッシュ」の概念が理解できず、ビルドのたびに全部やり直しになって遅かった。公式ドキュメントのベストプラクティスを読んでからは改善しました。

### 学習コストは1〜2日

基本的な使い方（run、build、compose up）を覚えるのに1日、Dockerfileを自分で書けるようになるのに2日程度でした。プログラミング自体の学習に比べれば、はるかに短い投資で大きなリターンが得られます。

![Dockerのポイントまとめ](images/point-docker-benefits.png)
