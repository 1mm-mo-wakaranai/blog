...
![GitHub Issues高速化 理解度の変化](images/comparison-before-after.png)
...
GitHub Issuesの高速化を体験して、自分のプロジェクトにも応用しようとした際の失敗談です。

### ハマり1: プリフェッチを入れすぎてサーバーに負荷をかけた

GitHubの仕組みに感動して、自分のWebアプリにもプリフェッチを実装。ページ内の全リンクを先読みする設定にしたら、1ページ表示するだけで50件以上のAPIリクエストが飛び、サーバーが悲鳴を上げました。

**改善:** プリフェッチは「ユーザーがクリックしそうなリンク」に限定する。マウスホバー時に発火させるか、ビューポート内のリンクだけに絞るのが正解。

### ハマり2: キャッシュの古いデータを信じてバグを見逃した

Issueのステータスが「Open」のままキャッシュされていて、実際にはチームメイトが「Closed」にしていたのに気づかず、同じバグを二重に修正してしまいました。Stale-While-Revalidateの「数秒のラグ」を理解していなかったのが原因。

**気づき:** キャッシュされたデータは「数秒前の状態」であることを意識する。重要な判断をする前はブラウザをリロードして最新状態を確認する。

### ハマり3: Service Workerのキャッシュが残り続けて開発中に混乱

自分のアプリでService Workerを使ったキャッシュを実装したら、コードを更新してもブラウザに古いバージョンが表示され続ける問題が発生。開発者ツールで「Update on reload」を有効にするまで30分悩みました。

**改善:** 開発中はService Workerを無効にするか、バージョニング戦略を最初から設計しておく。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
プリフェッチ入れすぎてサーバー落とすの、やりそう…。「必要なものだけ」が大事なんだね。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
GitHubも全Issueを先読みしてるわけじゃなくて、表示中の一覧に絞ってるんだ。「見えてるものだけ」がコツだよ。
{{< /chat >}}
