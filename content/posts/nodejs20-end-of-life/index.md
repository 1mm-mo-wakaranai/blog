...
Node.js 20がEOLになるって聞いたけど、今すぐアップデートしなきゃダメ？
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
セキュリティパッチが来なくなるから、計画的に移行した方がいいよ。移行手順と注意点をまとめたから参考にしてね。
{{< /chat >}}

![Node.js EOL 理解度の変化](images/comparison-before-after.png)
...
Node.jsのバージョン移行は「コマンド一発」に見えて、実際にやると地味なトラップがあります。筆者の失敗談を共有します。

### ハマり1: nvmでインストールしたのにバージョンが変わらない

`nvm install 22` を実行して「インストール完了」と表示されたのに、`node -v` がまだ `v20.x.x` のまま。原因は、新しいターミナルを開いていなかったこと。nvmはシェルの起動時にバージョンを読み込むので、既存のターミナルでは反映されません。さらに、`nvm alias default 22` を忘れていたため、新しいターミナルを開いても元のバージョンに戻ってしまいました。

**気づき:** `nvm install` だけでは不十分。`nvm alias default` でデフォルトを設定し、ターミナルを再起動するまでがセット。

### ハマり2: node_modulesを消さずにバージョンを上げた

Node.js 22に切り替えた後、そのまま `npm run dev` を実行したら謎のエラーが出ました。原因は、Node.js 20でビルドされたネイティブモジュール（`node-sass` や `sharp` など）がNode.js 22と互換性がなかったこと。`node_modules` を削除して `npm install` し直したら解決しました。
...
**改善:** それ以降、Node.jsのバージョンを変えたら「まず `node_modules` を消して再インストール」を鉄則にしています。

### ハマり3: CI/CDのバージョンを更新し忘れてデプロイが壊れた

ローカルではNode.js 22で問題なく動いていたのに、GitHub Actionsでテストが落ちる。原因は `.github/workflows/test.yml` の `node-version` がまだ `'20'` のままだったこと。ローカルとCIでNode.jsのバージョンが違うと、「手元では動くのにCIで落ちる」という最悪のパターンになります。

**気づき:** ローカルのバージョンを上げたら、同じ日にCI/CDの設定も更新する。後回しにすると確実に忘れる。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
node_modules消し忘れ、私もやりそう…。チェックリスト作っておこうかな。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
バージョン移行は「切り替え → node_modules再インストール → テスト → CI更新」の4ステップをセットで覚えておくと安心だよ。
{{< /chat >}}
...
{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
nvmでバージョン切り替えるだけなら思ったより簡単そう…！
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
バージョン切り替え自体は簡単。問題は依存パッケージの互換性だから、テストを回してから本番に適用しよう。
{{< /chat >}}
