...
## 実際にrejectedエラーでパニックになった！（筆者の失敗談）
...
A: `git fetch` はリモートの変更情報をダウンロードするだけで、ローカルのファイルは変更しません。`git pull` は `git fetch` + `git merge` を1コマンドで実行するものです。「まず情報だけ確認したい」ときは`fetch`、「取り込んで反映したい」ときは`pull`を使います。

### Q: このエラーが怖いので、毎回pushする前にpullした方がいいですか？

A: 良い習慣です。特にチーム開発では「作業を始める前にpull」「pushする前にpull」を癖にしておくと、rejectedエラーの発生率がグッと下がります。一人で開発している場合でも、複数のPCで作業する可能性があるなら同様にpullを先にするのが安全です。
...
- `git push -f` は個人ブランチ以外では絶対に使わない
- READMEを自動生成した場合は最初に`git pull`するのを忘れずに
- ブランチ名（main vs master）の不一致にも注意
...
1. **エラーメッセージを確認** → `rejected`なら2へ、`Permission denied`なら認証設定を確認
2. **`git pull origin main` を実行** → 成功したら3へ、conflictが出たら4へ
3. **`git push origin main` を再実行** → 通ればOK
4. **ファイルのconflictを手動で解決** → `git add .` → `git commit` → `git push`
5. **ブランチ名を確認** → `git branch -a` でローカルとリモートの名前が一致しているか

---
### あわせて読みたい
- [GitHubアカウントを作ったけど何すればいい？最初の使い方ガイド](/posts/github-what-is-it/)
- [コマンドラインが怖い人へ ― 覚えるコマンド5つだけ](/posts/command-line-scary/)
- [Gitブランチが分からない人へ ― 図解で理解する基本操作](/posts/git-branch-beginner/)
