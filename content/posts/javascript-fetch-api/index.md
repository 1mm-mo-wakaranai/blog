...
この「データください」→「はいどうぞ」のやり取りを、JavaScriptのコード1行で実行できるのが `fetch` です。
...
このHTMLファイルをブラウザで開くと、10人のユーザー名とメールアドレスが一覧表示されます。
...
サーバーがJSONではなくHTMLを返している場合に起きます。URLが正しいか確認してください。
