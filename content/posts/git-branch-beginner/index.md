...
ブランチって何のためにあるの？mainに直接コミットしちゃダメ？
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
mainに直接だと、壊れたコードがそのまま本番に出ちゃうリスクがあるんだ。ブランチを切れば安全に実験できるよ。
{{< /chat >}}


「ブランチって何？なんで必要なの？」

Gitを使い始めると必ず出てくる「ブランチ」。概念が分かりにくいですが、図で理解すればスッキリします。基本操作をコマンド付きで解説します。
...
作業を分離できるので、お互いの変更が干渉しません。Gitの基本的な仕組みについては[GitHubとは？アカウント作成から最初の使い方まで](/posts/github-what-is-it/)も参考にしてください。
...
`-b`（または `-c`）オプションを付けると、作成と切り替えを同時にやってくれます。ターミナル操作に不安がある方は、[コマンドラインが怖い人へ ― 最初に覚える10コマンド](/posts/command-line-scary/)で基本を押さえておくと安心です。
...
VS Codeを使っている場合は、コンフリクト箇所に「Accept Current Change」「Accept Incoming Change」「Accept Both Changes」のボタンが表示されるので、クリックするだけで解決できます。VS Codeの便利な操作については[VS Code最初に覚えるべき設定とショートカット10選](/posts/vscode-shortcuts-beginner/)でまとめています。
...
チームで開発する場合は、命名ルールを決めておくとスムーズです。ブランチで作業した内容をリモートに反映するときにエラーが出ることがあります。その場合は[git pushでrejectedエラーが出たときの対処法](/posts/git-first-push-error/)を確認してみてください。
...
### Q: ブランチを切り替えたら、さっきまで書いていたコードが消えました。大丈夫ですか？

A: 消えたわけではありません。元のブランチに切り替えれば戻ってきます。ただし、変更をコミットしていない場合は失われる可能性があるので、切り替え前に `git add` と `git commit` をしておきましょう。

### Q: `git checkout` と `git switch` はどちらを使えばいいですか？

A: Git 2.23以降であれば `git switch` が推奨されています。`checkout` はブランチ切り替え以外の機能も持っているため、用途が明確な `switch` の方が初心者には分かりやすいです。

### Q: マージした後、ブランチは削除しないとダメですか？

A: 削除しなくても動作に問題はありません。ただ、使い終わったブランチが増えると一覧が見づらくなるので、マージ済みのブランチは削除するのが一般的です。

### Q: リモートブランチとローカルブランチの違いは何ですか？

A: ローカルブランチはあなたのPC上にあるブランチ、リモートブランチはGitHub等のサーバー上にあるブランチです。`git push` でローカルの変更をリモートに反映し、`git pull` でリモートの変更をローカルに取り込みます。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
ブランチ切って作業→マージの流れ、やってみたら意外と簡単だった！
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
慣れると「ブランチなしでどうやって開発してたんだろう」って思うようになるよ。
{{< /chat >}}
