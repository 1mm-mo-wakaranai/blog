...
TypeScript 7のコンパイラがGoで書き直されたって本当？何が変わるの？
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
本当だよ。コンパイル速度が10倍以上速くなるんだ。大規模プロジェクトほど恩恵が大きいよ。
{{< /chat >}}
...
VS Codeでの体験については[VS Code 1.121アップデート記事](/posts/vscode-121-update/)でも触れています。
...
## 実際にTypeScript 7.0 Betaを試してみた！（筆者の体験）

筆者が実際に個人プロジェクト（Next.js製のブログ、約8000行）でTypeScript 7.0 Betaを試した体験をお伝えします。
...
インストールは通常のTypeScriptと同じ手順。特別な設定なしで動きました。

### 体感速度の変化

- **TypeScript 6.0**: `npx tsc --noEmit` に約8秒
- **TypeScript 7.0 Beta**: 同じコマンドで約1.2秒

**6.7倍の高速化**。体感としては「コマンド打った瞬間に結果が出る」レベル。

### VS Codeでの体験

TypeScript Native Preview拡張を入れると、エディタ上の型チェックも高速化されました。ファイル保存後の赤線表示が「一瞬」で出るように。以前は大きなファイルだと2〜3秒待つこともありましたが、ほぼリアルタイムになりました。

### 問題は出たか？

- ESLintのTypeScript pluginは問題なく動作
- Prettierも問題なし
- ただし、一部のカスタムtransformerプラグインが動かず`Cannot find module`エラー。Beta版なので想定内

### 結論

本番プロジェクトにはまだ使わないが、**個人プロジェクトで試す価値は十分にある**。正式リリースが楽しみ。

### 試す前に知っておきたいこと

- Beta版なので予期しないバグがある可能性がある。本番では使わない
- `package.json`の`devDependencies`に`typescript@beta`を追加するだけで試せる
- 元に戻したいときは`npm install -D typescript@latest`で安定版に戻せる
- VS Codeの拡張は通常のTypeScript拡張と共存できるので、切り替えが簡単
...
### Q: TypeScript 7.0のGo製コンパイラはオープンソースですか？

A: はい、TypeScriptは引き続きオープンソース（Apache-2.0ライセンス）で開発されています。Go版のソースコードもGitHubのMicrosoft/TypeScriptリポジトリで公開されています。

### Q: メモリ使用量はどう変わりますか？

A: Go版コンパイラはメモリ効率も改善されており、大規模プロジェクトではメモリ使用量が30〜50%削減される傾向があります。CI/CDのコンテナ環境（メモリ制限がある）では特に恩恵が大きいです。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
10倍速いのはやばい…！大きいプロジェクトのビルド待ち時間が消えるってこと？
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
かなり短縮されるよ。ただしまだベータだから、本番導入は安定版を待とう。エコシステムの対応状況も要チェック。
{{< /chat >}}
...
- [VS Code 1.121アップデートまとめ](/posts/vscode-121-update/)
