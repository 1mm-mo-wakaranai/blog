...
![Copilot Remote Control 理解度の変化](images/comparison-before-after.png)
...
Remote Controlを実際に使ってみて、便利な反面いくつか注意点がありました。

### ハマり1: /remote onを忘れて外出してしまった

Copilotに大きなリファクタリングを任せて外出したのに、`/remote on` を実行し忘れていました。スマホからセッションが見えず、Copilotが確認を求めて止まっている状態に。帰宅するまで作業が進まず、1時間無駄にしました。

**改善:** 長時間タスクを任せるときは、指示を出す前に `/remote on` を実行する習慣をつけました。

### ハマり2: スマホから指示を出したら意図と違う解釈をされた

スマホのフリック入力で「テストファイルも追加して」と送ったつもりが、変換ミスで曖昧な指示になり、Copilotが全く関係ないファイルを生成。PCに戻ってから `git checkout .` で戻しました。

**気づき:** スマホからの指示は短く明確に。曖昧な日本語は誤解されやすい。「src/auth/login.test.ts を作成して」のように具体的に書く。

### ハマり3: セッションの24時間制限を知らなかった

金曜の夜にRemote Controlを有効にして、土曜の夜にスマホから確認しようとしたら接続できない。24時間で自動的にリモート接続が無効化される仕様を知らなかったのが原因です。

**改善:** 長期間のタスクは24時間以内に完了するよう分割するか、翌日PCから再度 `/remote on` を実行する前提で計画を立てています。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
/remote onを忘れるの、絶対やりそう…。外出前チェックリストに入れておこう。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
長時間タスクを任せるときの手順を決めておくといいよ。「指示→/remote on→外出」の順番をルーティンにしよう。
{{< /chat >}}
