...
A: エージェントが生成するコードはgit work tree内で実行されるため、メインブランチへの直接変更はありません。ただし、テクニカルプレビュー段階なので、機密性の高いプロジェクトでの利用は慎重に検討してください。生成されたコードは必ずレビューしてからマージしましょう。

### Q: オフラインで使える？

A: いいえ。GitHub Copilotはクラウドのモデルを使っているため、インターネット接続が必要です。オフライン環境では動作しません。

### Q: チームで使う場合、料金はどうなる？

A: BusinessプランまたはEnterpriseプランでチーム利用が可能です。管理者がチームメンバーに対してCopilotのアクセスを付与する形式になります。個人のProプランでは自分のみの利用です。

### Q: デスクトップアプリとVS Code拡張は同時に使える？

A: はい、同時に使えます。デスクトップアプリで大きなタスクを実行しながら、VS Codeで別のファイルを編集するといった並行作業が可能です。GitHubアカウントは共有されるので、追加料金はかかりません。
...
- VS Codeの代替ではなく補完ツール。使い分けが重要
- Agent Mergeの結果は必ずdiffを確認する（テスト削除に注意）
- Freeプランでも試せるが、エージェント機能フル活用にはProプラン推奨
