...
### Q: VS Codeが重いと感じたらどうすればいいですか？

A: まず不要な拡張機能を無効化してみてください。`Ctrl+Shift+P` → 「Show Running Extensions」で各拡張機能の負荷が確認できます。また、大きなファイル（1万行以上）を開いている場合はミニマップをオフにすると改善することがあります。メモリ不足の場合はPCのスペックの問題なので、開くファイルやプロジェクトの数を減らしてみましょう。
...
- 拡張機能は必要になったら入れる（入れすぎ注意）
- コマンドパレット（Ctrl+Shift+P）は迷ったときの味方
- 自動保存を最初にオンにするだけでトラブルの半分は防げる

全部一度に覚えようとしなくて大丈夫です。毎日コードを書いていれば、自然と手が覚えます。

---

## 筆者おすすめのVS Code設定（すぐコピペで使える）

settings.jsonに以下を追加しておくと、初心者でも快適に使えます。`Ctrl+Shift+P` → 「Preferences: Open Settings (JSON)」で設定ファイルを開いて貼り付けてください。
...
| 設定 | 効果 |
|------|------|
| autoSave | 自動保存（保存忘れ防止） |
| fontSize: 14 | 文字サイズを少し大きく |
| tabSize: 2 | インデントをスペース2に |
| wordWrap: on | 長い行を折り返し表示 |
| minimap: false | 右のミニマップを非表示（画面が広くなる） |
| formatOnSave | 保存時に自動整形（Prettier必要） |
