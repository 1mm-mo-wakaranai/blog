...
![GitHubを活用する：次のステップ](images/step-guide.png)
...
### Q: GitHubの無料プランで何ができますか？
A: 無料プランでも主要な機能はほぼ全て使えます。Publicリポジトリ無制限、Privateリポジトリ無制限、GitHub Actions（CI/CD）月2,000分、GitHub Pages対応、コラボレーター招待も可能。個人開発や学習用途なら無料プランで全く問題ありません。有料プラン（Pro: 月4ドル）は高度なアクセス制御や追加の機能が必要な場合のみ検討すればOKです。
...
- Commit = 変更履歴付きの保存
- パスワードは絶対に上げない
- 最初はREADMEの編集から始めると安心
- 慣れてきたらローカルからpush、ブランチ操作へステップアップ

最初は「コードの保管場所」として使うだけで十分です。慣れてきたら、他の人のコードを見たり、自分のポートフォリオとして活用したりできます。

---

## 実際にGitHubを使い始めてみた！（筆者の体験）

筆者がGitHubを初めて使ったときの体験をお伝えします。

最初は「英語ばかりで何もわからん…」状態でしたが、やったことは3つだけ：

1. **アカウント作成**（5分）
2. **READMEを編集**（プロフィール用リポジトリを作って自己紹介を書いた）
3. **学習用リポジトリに毎日コミット**（「草を生やす」のモチベーションで続けられた）

特に「毎日コミットして緑のマス（草）を埋める」というのは、プログラミング学習のモチベーション維持に効きました。「今日もちゃんとコード書いたぞ」という可視化ができるのはGitHubならではの体験です。

### 初心者がまず作るべきリポジトリ3つ

1. **プロフィール用**（ユーザー名と同じ名前のリポジトリ）— 自己紹介が表示される
2. **学習メモ用**（`til` = Today I Learned）— 毎日学んだことを1行ずつ追加
3. **最初のプロジェクト用** — 実際に作ったアプリやスクリプトを置く
