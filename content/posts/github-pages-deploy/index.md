...
自分のサイトを無料で公開したいんだけど、GitHub Pagesってどうやって使うの？
{{< /chat >}}
...
![GitHub Pages 理解度の変化](images/comparison-before-after.png)
...
GitHub Pagesは簡単ですが、初めてだと意外なところでつまずきます。

### ハマり1: index.htmlを「Index.html」にしてしまった

ファイル名を `Index.html`（大文字始まり）にしてしまい、GitHub Pagesが認識せず404エラー。Windowsでは大文字小文字を区別しないので気づきにくいのですが、GitHub Pages（Linux環境）では区別されます。30分悩んでからファイル名を確認して解決。

**気づき:** ファイル名は必ず小文字の `index.html` にする。Windowsで開発している場合は特に注意。

### ハマり2: CSSのパスを絶対パスで書いて読み込めなかった

ローカルでは正常に表示されるのに、GitHub Pagesにデプロイしたらスタイルが全く当たらない。原因は `href="/css/style.css"` と絶対パスで書いていたこと。GitHub Pagesでは `https://ユーザー名.github.io/リポジトリ名/` がルートになるため、`/css/style.css` だとリポジトリ名が含まれず404になります。

**改善:** GitHub Pagesでは相対パス `./css/style.css` を使う。または、Hugoなどの静的サイトジェネレーターの `baseURL` 設定を正しく設定する。

### ハマり3: Pagesの設定でブランチを間違えた

デフォルトブランチが `main` なのに、Pages設定で `master` を選んでしまい、何も表示されない状態に。GitHubのデフォルトブランチ名が `master` から `main` に変わったことを知らなかったのが原因です。

**改善:** `git branch` でローカルのブランチ名を確認してから、Pages設定を行うようにしています。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
大文字小文字でハマるの、Windowsあるあるだね…。気をつけよう。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
ローカルで動いてもデプロイ先で動かないケースは多いよ。パスとファイル名は要チェック。
{{< /chat >}}
...
{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
pushしたら本当に公開された…！無料でこれはすごい。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
ポートフォリオやブログの公開に最適だよ。カスタムドメインも設定できるからね。
{{< /chat >}}
