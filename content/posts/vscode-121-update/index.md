...
![VS Code 1.121アップデート 理解度の変化](images/comparison-before-after.png)
...
VS Codeのアップデートは基本的にスムーズですが、新機能を使い始めるときに地味にハマることがあります。

### 失敗談1: Mermaid拡張機能と標準機能が競合してプレビューが二重表示

1.121にアップデートした後も、以前入れていた「Markdown Preview Mermaid Support」拡張機能をそのまま残していました。結果、Mermaid図がプレビューに2回表示されるという謎の現象が発生。拡張機能をアンインストールしたら直りました。

**気づき:** 標準搭載された機能と同じ役割の拡張機能は、アップデート後に整理する。拡張機能一覧を定期的に見直す習慣をつける。

### 失敗談2: ターミナル出力圧縮のせいでエラーの詳細が見えなくなった

`chat.tools.compressOutput.enabled` がデフォルトで有効になっていることを知らず、AIエージェントにテストを実行させたら「テスト失敗」としか表示されない。エラーの詳細が圧縮されて消えていました。デバッグ時は一時的にオフにする必要がありました。

**改善:** デバッグ時は `"chat.tools.compressOutput.enabled": false` に切り替える。通常時はトークン節約のためにオンのままにしておく。

### 失敗談3: リモートエージェントの設定でSSH鍵のパスを間違えて接続不能

リモートエージェント機能を試そうとして、SSH鍵のパスを間違えて設定。接続エラーが出るのに、エラーメッセージが「Connection refused」としか表示されず、鍵のパスが原因だと気づくのに時間がかかりました。

**気づき:** 実験的機能を試すときは、まず公式ドキュメントの手順通りに進める。エラーが出たら設定ファイルのパスや値を一つずつ確認する。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
拡張機能の整理って忘れがちだよね…。アップデートのたびにチェックしたほうがいいんだ。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
標準機能が増えるたびに不要な拡張機能も増えるからね。月1回くらい拡張機能一覧を見直すといいよ。
{{< /chat >}}
