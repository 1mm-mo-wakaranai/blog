...
![try-except入門 理解度の変化](images/comparison-before-after.png)
...
try-exceptは便利ですが、使い方を間違えると逆にバグの温床になります。僕の失敗談を紹介します。

### 失敗談1: except Exceptionで全部握りつぶして本番障害

開発中に「とりあえず動けばいい」と思って `except Exception: pass` を書きまくっていました。テスト環境では問題なく動いていたのに、本番でデータが消えるバグが発生。エラーが全て握りつぶされていたので、ログにも何も残っておらず原因特定に3日かかりました。

**気づき:** `except Exception: pass` は絶対に書かない。最低でもログに記録する。想定するエラーだけを具体的にキャッチする。

### 失敗談2: try文の範囲が広すぎて意図しないエラーまでキャッチ

ファイル読み込みのエラーだけキャッチしたかったのに、try文の中に20行以上のコードを入れてしまい、全然関係ないTypeErrorまでFileNotFoundErrorのexceptで処理されていました。結果、バグに気づくのが遅れました。

**改善:** try文の中には「エラーが起きる可能性がある最小限のコード」だけを入れる。処理が長い場合は関数に分割する。

### 失敗談3: finallyでreturnして例外が消えた

finally文の中で `return` を書いたら、except文で `raise` したはずの例外が消えてしまいました。finallyのreturnが優先されるというPythonの仕様を知らず、「なぜエラーが上位に伝わらないのか」と半日悩みました。

**気づき:** finally文の中ではreturnを書かない。finallyはリソースの後片付け（ファイルを閉じる等）だけに使う。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
except Exceptionで全部キャッチするのって楽だけど、バグを隠しちゃうんだ…。怖い。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
「動いてるように見えるけど実は壊れてる」が一番怖いパターンだよ。エラーは隠さず、適切に処理しよう。
{{< /chat >}}
