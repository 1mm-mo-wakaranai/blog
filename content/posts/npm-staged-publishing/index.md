...
![npm staged publishing 理解度の変化](images/comparison-before-after.png)
...
staged publishingを実際に試してみて、いくつか引っかかったポイントがあります。

### ハマり1: 2FAを設定していなくてstage enableが通らない

`npm stage enable` を実行したら「2FA required」のエラー。staged publishingの有効化には2FA（二要素認証）が必須条件でした。npmアカウントの設定画面から2FAを有効にして、認証アプリ（Google Authenticatorなど）を設定してから再実行したら通りました。

**気づき:** staged publishingに限らず、npmでパッケージを公開するなら2FAは必須。セキュリティ機能を使うための前提条件として最初に設定しておくべき。

### ハマり2: CI/CDで`npm publish`した後に承認を忘れた

GitHub Actionsで自動的に`npm publish`が走り、ステージング状態になったのに、承認するのを忘れて3日間放置してしまいました。その間、ユーザーから「最新バージョンがインストールできない」と問い合わせが来て初めて気づきました。

```bash
# ステージング中のパッケージを確認するコマンド
npm stage list
...
**改善:** リリース後にSlack通知を飛ばすようにして、承認忘れを防止。CI/CDのワークフローに「承認待ちリマインダー」を組み込んだ。

### ハマり3: `npm stage approve`を別のマシンから実行しようとした

自宅PCで`npm publish`した後、外出先のスマホから承認しようとしたら、npm CLIが必要で実行できませんでした。現時点ではWebのダッシュボードからの承認UIは限定的で、基本的にはCLIからの操作が必要です。

**気づき:** 承認操作はnpm CLIが使える環境でしかできない。リリース作業は「公開＋承認」をセットで行う時間を確保しておく。

{{< chat name="初心者ちゃん" icon="/images/rin-icon.png" direction="left" >}}
承認忘れは怖い…。通知の仕組みを作っておくのが大事なんだね。
{{< /chat >}}

{{< chat name="全知全能くん" icon="/images/zenchi-icon.png" direction="right" >}}
セキュリティと利便性のトレードオフだね。承認ステップが増える分、運用フローをしっかり決めておくのがポイントだよ。
{{< /chat >}}
